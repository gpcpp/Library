...
public class ReaderController {
    ...
    @RequestMapping("regis_add.html")
    public ModelAndView regisAdd() {
        return new ModelAndView("regis_add");
    }

    @RequestMapping("regis_add_do.html")
    public String regisAddDo(String name, String sex, String birth, String address, String phone, String password, RedirectAttributes redirectAttributes) {
        ReaderInfo readerInfo = getReaderInfo(0, name, sex, birth, address, phone);
        long readerId = readerInfoService.addReaderInfo(readerInfo);
        readerInfo.setReaderId(readerId);
        if (readerId > 0 && readerCardService.addReaderCard(readerInfo, password)) {
